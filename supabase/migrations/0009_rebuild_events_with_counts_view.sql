-- Rebuild events_with_counts view with explicit column list so that columns
-- added after the initial migration (group_id, is_verified_organiser) are
-- visible. DROP + CREATE is required because CREATE OR REPLACE cannot change
-- the column order of an existing view.
